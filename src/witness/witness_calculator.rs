...
impl WitnessCalculator {
    ...
    pub fn from_module(module: Module, mut store: Store) -> Result<Self> {
        ...
        fn new_circom1(
            ...
            let prime = safe_memory.read_big(&mut store, ptr as usize, n32 as usize)?;
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "circom-2")] {
                match version {
                    2 => new_circom2(instance, memory, version, store),
                    1 => new_circom1(instance, memory, version, store),
    ...
    fn calculate_witness_circom1<I: IntoIterator<Item = (String, Vec<BigInt>)>>(
        ...
        for (name, values) in inputs.into_iter() {
            ...
            for (i, value) in values.into_iter().enumerate() {
                self.memory
                    .write_fr(&mut self.store, p_fr as usize, &value)?;
                self.instance
...
mod tests {
    ...
    fn run_test(case: TestCase) {
        ...
        assert_eq!(
            { wtns.instance.get_n_vars(&mut wtns.store).unwrap() },
            case.n_vars
        );
