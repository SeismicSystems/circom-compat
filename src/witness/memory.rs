...
impl SafeMemory {
    ...
    pub fn read_fr(&self, store: &impl AsStoreRef, ptr: usize) -> Result<BigInt> {
        // let view = self.memory.view::<u8>();
        let view = self.view(store);
        let view = unsafe { view.data_unchecked_mut() };

        let res = if view[ptr + 4 + 3] & 0x80 != 0 {
            let mut num = self.read_big(store, ptr + 8, self.n32)?;
            if view[ptr + 4 + 3] & 0x40 != 0 {
                num = (num * &self.r_inv) % &self.prime
            }
            num
        } else if view[ptr + 3] & 0x40 != 0 {
            let mut num = self.read_u32(store, ptr).into();
            // handle small negative
            num -= BigInt::from(0x100000000i64);
            num
        } else {
            self.read_u32(store, ptr).into()
        };

        Ok(res)
    }

    fn write_short_positive(
        &mut self,
    ...
    fn write_big(&self, store: &impl AsStoreRef, ptr: usize, num: &BigInt) -> Result<()> {
        let view = self.view(store);
        let buf = unsafe { view.data_unchecked_mut() };
